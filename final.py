...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    delta_name = ['-','^','v']
    delta = [[0,1], [-1,1],[1,1]]
    cost = [[999. for col in road[0]] for row in road]
    path = [[' ' for col in road[0]] for row in road]
    change = True
            ...
            for j in range(len(cost[0])):
              if goal[0] == i and goal[1] == j:
                ...
                for a in range(len(delta)):
                  ...
                  if i2 >= 0 and j2 >= 0 and i2 < len(cost) and j2 < len(cost[0]) and road[i2][j2] != 0:
                    movement_cost = (1. / road[i][j])
                    v2 = cost[i2][j2] + movement_cost
                    if i != i2:
                      v2 += lane_change_cost

                    if v2 < cost[i][j]:
                      change = True
                      path[i][j] = delta_name[a]
                      cost[i][j] = v2

...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        distance_moved = motion[1] + random.gauss(0.0, self.distance_noise)
        alpha = motion[0] + random.gauss(0.0, self.steering_noise)# steering angle
        beta = distance_moved / self.length * tan(alpha) # turning angle
        theta = self.orientation

        if(beta >= 0.001):
            R = distance_moved / beta # turning radius
            cx = self.x- sin(theta) * R # turning radius point
            ...
            new_x = self.x + (distance_moved * cos(theta))
            new_y = self.y + (distance_moved * sin(theta))
            new_theta = theta
    ...
    def sense(self, flag=1): #do not change the name of this function
      ...
      for i in range(len(landmarks)):
        ...
        if(flag==1):
              new_z += random.gauss(0.0, self.bearing_noise)
        Z.append( new_z % (2*pi) )

...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # expansion mapping
    expand = [0,1]
    for i in range(4,len(omega.value)+2):
        expand.append(i)

    for k in range(len(data)):
        n =0
        # expand the matrix by 2 for x2,y2
        omega = omega.expand(dim+2,dim+2,expand)
        xi = xi.expand(dim+2, 1, expand, [0])

        
        measurement = data[k][0]
        motion = data[k][1]
        #print 'measure',len(measurement)
        for i in range(len(measurement)):
            #print measurement[i]
            # m is the index of the landmark coordinate in the matrix/vector
            m = 4+ (2*measurement[i][0])
            #print 'measure',measurement[i][0], m
            #print 'i', i
            # update the information maxtrix/vector based on the measurement
            ...
            for b in range(2):
                ...
                xi.value[n+b][0]      += -measurement[i][1+b] / measurement_noise
                xi.value[m+b][0]      +=  measurement[i][1+b] / measurement_noise
                #print 'xiii', m+b, xi.value[m+b][0], measurement[i][1+b] / measurement_noise
        # update the information maxtrix/vector based on the robot motion
        for b in range(4):
            # correct
        ...
        #mu = omega_inverse * xi
        # now take
        inverse_b = omega.take([0,1]).inverse()
        c_xi = xi.take([0,1],[0])

        # 2, 3, 4, ... n
        xi_prime = xi.take([x for x in range(2,len(xi.value))],[0])

        ma = omega.take([0,1],[x for x in range(2,len(omega.value))])
        ma_transpose = ma.transpose()

        # 2_0, 2_1, 3_0, 3_1, 4_0, 4_1, ... n
        
        omega_prime = omega.take([x for x in range(2,len(omega.value))])
        
        omega = omega_prime - (ma_transpose * inverse_b * ma)
        xi = xi_prime - (ma_transpose * inverse_b * c_xi)
        
        #print 'omega',  len(omega.value), len(omega.value[0])
        #print 'xi',  len(xi.value), len(xi.value[0])

    #omega.show('final')
    #print 'invomega',  len(invomega.value), len(invomega.value[0])
    #xi.show('test')
    mu = omega.inverse() * xi
...
#print_result(N, num_landmarks, result[0])
